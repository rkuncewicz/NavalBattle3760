...
/*uncomment this if they send a speed value from 0-1 instead of 0-130
...
public void moveShips()
{
  ...
  // double adjustedspeed = getAdjustedSpeed(); //Uncomment this line if the server does NOT sync up with our client. It may prove that we are right and they are wrong.
   double xadjust = 0;
   double yadjust = 0;
   int currentx = 0;
   int currenty = 0;
...
public void computeVector(int i){
     ...
     dybydx = (double)dy/(double)dx;//Ratio of y to x
        
        
           head = (head * -1) + 90;//Transform our heading into something that Java.Math can use
           if(head < 0)
               head = head + 360;//No negatives!
     ...
     {//Vector is perpendicular-left to the heading of the ship take the negative partial negative reicprocal
     ...
     {//Vector is perpendicular-right to the heading of the ship. Take the negative reciprocal
 ...
 public void drawCannonballs(int i, Graphics g){
    ...
    game.shiplist.get(i).cball.x = game.shiplist.get(i).cball.x + (int)((game.shiplist.get(i).cballVectorx/framesPerSecond)*cannonballSpeed);  //Move the cannonball in the reqiuired direction
